class DetailPanel {
  ...
  renderInformasiUmum(state, activeIds) {
    ...
    if (state.selectedVillageId && state.rawData.wilayah[state.selectedVillageId]) {
      ...
      const demo = state.rawData.demografi.find(x => x.id_desa === v.id_desa && Number(x.tahun) === 2025) || {};
      ...
      this.contentEl.innerHTML = `
        <div class="detail-grid-cards" style="margin-bottom: 20px;">
          ...
          <div class="info-profile-card">
            ...
            <span class="profile-card-val" style="font-size: 1.1rem; font-family: monospace;">${Number(v.latitude || 0).toFixed(5)}, ${Number(v.longitude || 0).toFixed(5)}</span>
          </div>
        </div>
  ...
  // Tab 4: Data Kapal (Line chart + yearly table)
  renderKapal(state, activeIds) {
    this.panelTitleEl.innerHTML = `<i class="fa-solid fa-ship"></i> Tren Armada Kapal Perikanan`;

    const history = DataManager.getVesselsHistory(activeIds, state.rawData);
    ...
    this.contentEl.innerHTML = `
      <div class="detail-dual-layout">
        <div class="chart-container" style="background-color:white; min-height: 250px; position:relative;">
          <h3 class="chart-title">Tren Perkembangan Armada Kapal</h3>
        ...
        <div class="detail-table-wrapper">
          <table class="detail-table">
            <thead>
              <tr>
                <th>Tahun</th>
                <th>Kapal Motor (Unit)</th>
                <th>Kapal Kayu (Unit)</th>
                <th>Total Kapal (Unit)</th>
              </tr>
            </thead>
            <tbody>
              ${history.map(row => {
                const motor = row.types["KAPAL MOTOR"] || 0;
                const kayu = row.types["KAPAL KAYU"] || 0;
                return `
                  <tr>
                    <td><strong>${row.year}</strong></td>
                    <td>${motor > 0 ? this.formatNumber(motor) : "-"}</td>
                    <td>${kayu > 0 ? this.formatNumber(kayu) : "-"}</td>
                    <td><strong>${this.formatNumber(row.total)} Unit</strong></td>
                  </tr>
                `;
              }).join("")}
            </tbody>
    ...
    this.detailChart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [
          ...
          {
            label: "Kapal Motor",
            data: history.map(h => h.types["KAPAL MOTOR"] || 0),
            borderColor: "#00b4d8",
            borderDash: [5, 5],
            fill: false,
            tension: 0.3,
          ...
          {
            label: "Kapal Kayu",
            data: history.map(h => h.types["KAPAL KAYU"] || 0),
            borderColor: "#e36414",
            borderDash: [5, 5],
            fill: false,
            tension: 0.3,
  ...
  // Tab 5: Alat Tangkap (Line chart + yearly table)
  renderAlatTangkap(state, activeIds) {
    this.panelTitleEl.innerHTML = `<i class="fa-solid fa-anchor"></i> Tren Alat Tangkap Perikanan`;

    const history = DataManager.getGearsHistory(activeIds, state.rawData);
    ...
    this.contentEl.innerHTML = `
      <div class="detail-dual-layout">
        <div class="chart-container" style="background-color:white; min-height: 250px; position:relative;">
          <h3 class="chart-title">Tren Perkembangan Alat Tangkap</h3>
          ...
          <table class="detail-table">
            <thead>
              <tr>
                ...
                <th>Bubu (Unit)</th>
                <th>Cantrang (Unit)</th>
                <th>Total Alat Tangkap (Unit)</th>
              </tr>
            </thead>
            <tbody>
              ${history.map(row => {
                const pancing = row.types["PANCING"] || 0;
                const bubu = row.types["BUBU"] || 0;
                const cantrang = row.types["CANTRANG"] || 0;
                return `
                  <tr>
                    <td><strong>${row.year}</strong></td>
                    <td>${pancing > 0 ? this.formatNumber(pancing) : "-"}</td>
                    <td>${bubu > 0 ? this.formatNumber(bubu) : "-"}</td>
                    <td>${cantrang > 0 ? this.formatNumber(cantrang) : "-"}</td>
                    <td><strong>${this.formatNumber(row.total)} Unit</strong></td>
                  </tr>
                `;
              }).join("")}
    ...
    this.detailChart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [
          ...
          {
            label: "Pancing",
            data: history.map(h => h.types["PANCING"] || 0),
            borderColor: "#e36414",
            borderDash: [5, 5],
            fill: false,
            tension: 0.3,
          ...
          {
            label: "Bubu",
            data: history.map(h => h.types["BUBU"] || 0),
            borderColor: "#2ec4b6",
            borderDash: [5, 5],
            fill: false,
            tension: 0.3,
          ...
          {
            label: "Cantrang",
            data: history.map(h => h.types["CANTRANG"] || 0),
            borderColor: "#ffb703",
            borderDash: [5, 5],
            fill: false,
            tension: 0.3,
  ...
  // Tab 7: Infrastruktur (Line chart + yearly table)
  renderInfrastruktur(state, activeIds) {
    this.panelTitleEl.innerHTML = `<i class="fa-solid fa-warehouse"></i> Tren Infrastruktur Perikanan`;

    const history = DataManager.getInfrastructureHistory(activeIds, state.rawData);

    if (history.length === 0) {
      this.contentEl.innerHTML = `<p class="empty-state"><i class="fa-solid fa-circle-question"></i> Tidak ada data infrastruktur tersedia.</p>`;
      return;
    }
    ...
    this.contentEl.innerHTML = `
      <div class="detail-dual-layout">
        <div class="chart-container" style="background-color:white; min-height: 250px; position:relative;">
          <h3 class="chart-title">Tren Perkembangan Infrastruktur</h3>
          <canvas id="chart-detail-infra" style="max-height:220px;"></canvas>
        </div>
        <div class="detail-table-wrapper">
          <table class="detail-table">
            <thead>
              <tr>
                <th>Tahun</th>
                <th>Cold Storage (Unit)</th>
                <th>Dermaga Apung (Unit)</th>
                <th>Rumah Produksi (Unit)</th>
                <th>Total Infrastruktur (Unit)</th>
              </tr>
            </thead>
            <tbody>
              ${history.map(row => {
                const cs = row.types["COLD STORAGE"] || 0;
                const da = row.types["DERMAGA APUNG"] || 0;
                const rp = row.types["RUMAH PRODUKSI"] || 0;
                return `
                  <tr>
                    <td><strong>${row.year}</strong></td>
                    <td>${cs > 0 ? this.formatNumber(cs) : "-"}</td>
                    <td>${da > 0 ? this.formatNumber(da) : "-"}</td>
                    <td>${rp > 0 ? this.formatNumber(rp) : "-"}</td>
                    <td><strong>${this.formatNumber(row.total)} Unit</strong></td>
                  </tr>
                `;
              }).join("")}
    ...
    // Render Line Chart
    const ctx = document.getElementById("chart-detail-infra").getContext("2d");
    this.detailChart = new Chart(ctx, {
      type: "line",
      data: {
        labels: history.map(h => h.year),
        datasets: [
          ...
          {
            label: "Cold Storage",
            data: history.map(h => h.types["COLD STORAGE"] || 0),
            borderColor: "#2ec4b6",
            borderDash: [5, 5],
            fill: false,
            tension: 0.3,
          ...
          {
            label: "Dermaga Apung",
            data: history.map(h => h.types["DERMAGA APUNG"] || 0),
            borderColor: "#e36414",
            borderDash: [5, 5],
            fill: false,
            tension: 0.3,
          ...
          },
          {
            ...
            data: history.map(h => h.types["RUMAH PRODUKSI"] || 0),
            borderColor: "#ffb703",
            borderDash: [5, 5],
            fill: false,
            tension: 0.3,
