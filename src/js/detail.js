class DetailPanel {
  ...
  renderKapal(state, activeIds) {
    ...
    // Get table records
    const shipRecords = state.rawData.data_kapal.filter(x => activeIds.includes(x.id_desa) && Number(x.tahun) === 2025);
  ...
  renderAlatTangkap(state, activeIds) {
    ...
    const gearRecords = state.rawData.data_alat_tangkap.filter(x => activeIds.includes(x.id_desa) && Number(x.tahun) === 2025);
