const DataManager = {
  ...
  filterData(rawData, filters) {
    ...
    return Object.values(rawData.wilayah).filter(village => {
      ...
      if (filters.kawasan && village.keterangan_kk !== filters.kawasan) {
        return false;
      }
  ...
  // Get Vessel history by active scope
  getVesselsHistory(activeIds, rawData) {
    const history = {}; // year -> { total: 0, types: { [type]: count } }
    if (!rawData || !rawData.data_kapal) return [];

    rawData.data_kapal.forEach(record => {
      if (activeIds.includes(record.id_desa)) {
        const yr = parseInt(record.tahun);
        if (isNaN(yr)) return;
        if (!history[yr]) {
          history[yr] = { total: 0, types: {} };
        }
        const qty = record.jumlah || 0;
        history[yr].total += qty;
        
        const type = record.jenis || "Lainnya";
        history[yr].types[type] = (history[yr].types[type] || 0) + qty;
      }
    });
  ...
  // Get Gears history by active scope
  getGearsHistory(activeIds, rawData) {
    const history = {}; // year -> { total: 0, types: { [type]: count } }
    if (!rawData || !rawData.data_alat_tangkap) return [];
  ...
  // Get Infrastructure history by active scope
  getInfrastructureHistory(activeIds, rawData) {
    const history = {}; // year -> { total: 0, types: { [type]: count } }
    if (!rawData || !rawData.data_infrastruktur) return [];

    rawData.data_infrastruktur.forEach(record => {
      if (activeIds.includes(record.id_desa)) {
        const yr = parseInt(record.tahun);
        if (isNaN(yr)) return;
        if (!history[yr]) {
