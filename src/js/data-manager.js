const DataManager = {
  ...
  getAggregatedMetrics(filteredData, rawData) {
    ...
    if (rawData.demografi) {
      rawData.demografi.forEach(record => {
        if (activeIds.includes(record.id_desa) && record.tahun === 2025) {
  ...
  getInfrastructureData(activeIds, rawData) {
    ...
    return rawData.data_infrastruktur.filter(record => activeIds.includes(record.id_desa) && record.tahun === 2025);
  }
};
